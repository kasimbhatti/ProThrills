...
def main():
    ...
    while rows < 5 or columns < 5 or height < 5:
        print("The board should be at least 5 x 5 x 5 !")
    ...
    print("Graphics by Kasim Rehman Bhatti, game mechanics by Atharva ..., server hosting by Tillega ...")
    ...
    while not glfw.window_should_close(window) and not Done:
        ...
        if currentNumberBuffer[0] != -1 and currentNumberBuffer[1] != -1 and not connectFound:
            ...
            if EnteredRow and EnteredColumn:
                ...
                if NewGame.newTurn(currentNumberBuffer[0], currentNumberBuffer[1], newHeight) == 1:
                    connectFound = True
                    startBuffer = [currentNumberBuffer[0], currentNumberBuffer[1], newHeight]
        ...
        if connectFound:
            if startBuffer[0] != -1 and startBuffer[1] != -1 and startBuffer[2] != -1:
                ...
                a = [objectCentre[0] + ((startBuffer[0] - int(rows / 2)) * length),
                     objectCentre[1] + (startBuffer[2] * multiplier) + 0.1,
                ...
                d = [objectCentre[0] + ((endBuffer[0] - int(rows / 2)) * length),
                     objectCentre[1] + (endBuffer[2] * multiplier) + 0.1,
